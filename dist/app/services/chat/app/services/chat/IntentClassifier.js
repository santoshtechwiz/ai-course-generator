...
const NlpIntentClassifier_1 = require("./NlpIntentClassifier");
const chat_types_1 = require("../../../types/chat.types");
class IntentClassifier {
    constructor() {
        this.patterns = {};
        this.openai = new openai_1.default({ apiKey: process.env.OPENAI_API_KEY });
        this.nlpClassifier = new NlpIntentClassifier_1.NlpIntentClassifier('./models/chat-intents.nlp');
    ...
    initializePatterns() {
        this.patterns = {
            [chat_types_1.ChatIntent.NAVIGATE_COURSE]: [
            ...
            [chat_types_1.ChatIntent.NAVIGATE_QUIZ]: [
            ...
            [chat_types_1.ChatIntent.CREATE_QUIZ]: [
            ...
            [chat_types_1.ChatIntent.CREATE_COURSE]: [
            ...
            [chat_types_1.ChatIntent.EXPLAIN_CONCEPT]: [
            ...
            [chat_types_1.ChatIntent.TROUBLESHOOT]: [
            ...
            [chat_types_1.ChatIntent.SUBSCRIPTION_INFO]: [
            ...
            [chat_types_1.ChatIntent.GENERAL_HELP]: [
            ...
            [chat_types_1.ChatIntent.OFF_TOPIC]: [
    ...
    async initializeNlpClassifier() {
        try {
            ...
            const { DEFAULT_INTENTS } = await Promise.resolve().then(() => require('../../../config/intent-config'));
            for (const [intentName, config] of Object.entries(DEFAULT_INTENTS)) {
                this.nlpClassifier.addIntent(intentName, config.utterances, config.responses);
            }
    ...
    async classify(message, context) {
        ...
        if (simpleGreeting.test(message)) {
            return {
                intent: chat_types_1.ChatIntent.OFF_TOPIC,
                confidence: 0.95,
                entities: {
                    quantity: 0,
                },
        ...
        if (entities.topics && entities.topics.length > 0) {
            ...
            const hasCoursePattern = patternMatches.some(m => m.intent === chat_types_1.ChatIntent.NAVIGATE_COURSE);
            const hasQuizPattern = patternMatches.some(m => m.intent === chat_types_1.ChatIntent.NAVIGATE_QUIZ);
            const hasCreateQuizPattern = patternMatches.some(m => m.intent === chat_types_1.ChatIntent.CREATE_QUIZ);
            const hasCreateCoursePattern = patternMatches.some(m => m.intent === chat_types_1.ChatIntent.CREATE_COURSE);
            const hasExplainPattern = patternMatches.some(m => m.intent === chat_types_1.ChatIntent.EXPLAIN_CONCEPT);
            if (hasCoursePattern) {
                console.log(`[IntentClassifier] Topic + course pattern → navigate_course (0.95)`);
                return {
                    intent: chat_types_1.ChatIntent.NAVIGATE_COURSE,
                    confidence: 0.95,
                    entities,
                };
            }
                ...
                return {
                    intent: chat_types_1.ChatIntent.CREATE_COURSE,
                    confidence: 0.95,
                    entities,
                };
            }
                ...
                return {
                    intent: chat_types_1.ChatIntent.CREATE_QUIZ,
                    confidence: 0.95,
                    entities,
                };
            }
                ...
                return {
                    intent: chat_types_1.ChatIntent.NAVIGATE_QUIZ,
                    confidence: 0.95,
                    entities,
                };
            }
                ...
                return {
                    intent: chat_types_1.ChatIntent.EXPLAIN_CONCEPT,
                    confidence: 0.90,
                    entities,
                };
            }
        ...
        if (patternMatches.length === 0 || patternMatches[0].confidence < 0.65) {
            ...
            try {
                ...
                const intentMap = {
                    'greeting': chat_types_1.ChatIntent.OFF_TOPIC,
                    'course.search': chat_types_1.ChatIntent.NAVIGATE_COURSE,
                    'quiz.start': chat_types_1.ChatIntent.NAVIGATE_QUIZ,
                    'course.create': chat_types_1.ChatIntent.CREATE_COURSE,
                    'help.general': chat_types_1.ChatIntent.GENERAL_HELP,
                    'unknown': chat_types_1.ChatIntent.GENERAL_HELP, // Map unknown to general help
                };
                const mappedIntent = intentMap[nlpResult.intent] || chat_types_1.ChatIntent.GENERAL_HELP;
        ...
        for (const [intentKey, patterns] of Object.entries(this.patterns)) {
            ...
            for (const pattern of patterns) {
                if (pattern.test(message)) {
                    let confidence = 0.75;
                    if (intent === chat_types_1.ChatIntent.NAVIGATE_COURSE) {
                    ...
                    else if (intent === chat_types_1.ChatIntent.EXPLAIN_CONCEPT) {
                        if (/\b(what|why|how|explain|confused|don't understand)\b/i.test(message)) {
                            confidence += 0.08;
                        }
                    }
                    else if (intent === chat_types_1.ChatIntent.TROUBLESHOOT) {
...
${Object.values(chat_types_1.ChatIntent).join('\n- ')}
...
Respond with JSON only.`,
                ...
                functions: [
                    {
                        ...
                        parameters: {
                            ...
                            properties: {
                                intent: {
                                    type: 'string',
                                    enum: Object.values(chat_types_1.ChatIntent)
                                },
                                confidence: {
                                    type: 'number',
                                    minimum: 0,
    ...
    getDefaultIntent() {
        return {
            intent: chat_types_1.ChatIntent.GENERAL_HELP,
            confidence: 0.50,
            entities: {
                quantity: 0,
            },
     ...
     */
    isRelevantQuery(intent) {
        return intent !== chat_types_1.ChatIntent.OFF_TOPIC;
    }
}
