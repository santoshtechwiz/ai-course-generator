  ...
  for (const [key, synonyms] of Object.entries(PROGRAMMING_SYNONYMS)) {
    if (words.some((word) => word.includes(key))) {
      synonyms.forEach((synonym) => {
        expanded.push(
          text.toLowerCase().replace(new RegExp(key, 'gi'), synonym)
        );
      });
    }
  }
...
export function calculateAnswerSimilarity(
  ...
  const levenshtein =
    1 -
    levenshteinDistance(user.toLowerCase(), correct.toLowerCase()) /
  ...
  const similarity =
    levenshtein * 0.2 + jaroWinkler * 0.3 + wordSim * 0.3 + semanticSim * 0.2;
