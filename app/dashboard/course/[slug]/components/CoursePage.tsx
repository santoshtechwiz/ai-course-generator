...
type CoursePageAction =
  | { type: "SET_SIDEBAR_OPEN"; payload: boolean }
  | { type: "SET_SMALL_SCREEN"; payload: boolean }
  | { type: "SET_COMPLETION_OVERLAY"; payload: boolean }
  | { type: "SET_FORCE_SHOW_CONTENT"; payload: boolean }

const coursePageReducer = (state: CoursePageState, action: CoursePageAction): CoursePageState => {
  switch (action.type) {
    case "SET_SIDEBAR_OPEN":
      return { ...state, isSidebarOpen: action.payload }
    case "SET_SMALL_SCREEN":
      return { ...state, isSmallScreen: action.payload }
    case "SET_COMPLETION_OVERLAY":
      return { ...state, showCompletionOverlay: action.payload }
    case "SET_FORCE_SHOW_CONTENT":
      return { ...state, forceShowContent: action.payload }
    default:
      return state
  }
}

// Custom Hook for Video Playlist with memoization
...
// Debounced progress update hook
...
export default function CoursePage({
  ...
  const [state, dispatchState] = useReducer(coursePageReducer, {
    isSidebarOpen: false,
    isSmallScreen: false,
  ...
  const autoPlay = useAppSelector((state) => state.course.autoplayEnabled)
  ...
  const debouncedUpdateProgress = useDebounce(updateProgress, 1000)
  ...
  const processPendingProgressUpdates = useCallback(() => {
    ...
    if (sessionUserIdRef.current && latestUpdate) {
      ...
      updateProgressRef.current(latestUpdate).catch((error: Error) => {
  ...
  // Optimized throttled update progress
  const throttledUpdateProgress = useCallback(
    (updateData: {
      ...
      const isCriticalUpdate = updateData.isCompleted === true || updateData.lastAccessedAt !== undefined
  ...
  // Optimized chapter completion handler
  const markChapterAsCompleted = useCallback(() => {
    ...
    const isAllChaptersCompleted = videoPlaylist.length > 0 && updatedCompletedChapters.length >= videoPlaylist.length
  ...
  const handleVideoEnd = useCallback(() => {
    ...
    if (nextVideoId) {
      ...
      dispatch(setCourseCompletionStatus(true))
      dispatchState({ type: "SET_COMPLETION_OVERLAY", payload: true })
    }
  }, [nextVideoId, dispatch, markChapterAsCompleted, isLastVideo, currentChapter])

  // Optimized video selection handler with race condition prevention
  ...
  // Optimized completion overlay handlers
  const handleWatchAnotherCourse = useCallback(() => {
    router.push("/dashboard/courses")
  }, [router])

  const handleCloseCompletionOverlay = useCallback(() => {
    dispatchState({ type: "SET_COMPLETION_OVERLAY", payload: false })
  }, [])

  // Optimized related courses fetcher
  ...
  const initialOrFirstVideoId = useMemo(
    () => currentVideoId || videoPlaylist[0]?.videoId,
    [currentVideoId, videoPlaylist],
  )
  ...
  // Optimized progress update with smarter change detection
  useEffect(() => {
    ...
    if (chapterId !== lastChapterIdRef.current || now - lastTimeProgressRef.current > 30000) {
  ...
  // Auto-timeout to force content display if loading gets stuck
  useEffect(() => {
    ...
    forceShowTimeoutRef.current = setTimeout(() => {
      if (isLoading || isProgressLoading) {
        ...
        dispatchState({ type: "SET_FORCE_SHOW_CONTENT", payload: true })
      }
    }, 5000)
  ...
  // Optimized responsive sidebar handling
  useEffect(() => {
    const handleResize = () => {
      const smallScreen = window.innerWidth < 1024
      dispatchState({ type: "SET_SMALL_SCREEN", payload: smallScreen })

      // Only auto-close on small screens when first loading
      if (!state.isSidebarOpen && !smallScreen) {
        dispatchState({ type: "SET_SIDEBAR_OPEN", payload: true })
      } else if (smallScreen && state.isSidebarOpen) {
        dispatchState({ type: "SET_SIDEBAR_OPEN", payload: false })
      }
    }
  ...
  // Define a stable onChapterComplete callback to avoid ReferenceError
  ...
  const shouldShowSkeleton = (isLoading || isProfileLoading) && !state.forceShowContent && !profileError
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:hidden">
        <MobilePlayList
          ...
          onSidebarOpen={() => dispatchState({ type: "SET_SIDEBAR_OPEN", payload: true })}
        />
      </div>
        ...
        <main className="flex-1 overflow-auto">
          <div className="container max-w-4xl py-6 lg:py-8 px-4 sm:px-6 lg:px-8 mx-auto">
            <div className="space-y-8">
              <MainContent
                slug={course.slug}
                initialVideoId={initialOrFirstVideoId}
                nextVideoId={nextVideoId}
                prevVideoId={prevVideoId}
                onVideoEnd={handleVideoEnd}
                onVideoSelect={handleVideoSelect}
                handlePlaylistVideoSelect={handleVideoSelect}
                ...
                course={course}
                relatedCourses={[]}
                isProgressLoading={isProgressLoading && !state.forceShowContent}
              ...
              {state.isSmallScreen && (
                <div className="border-b p-4">
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => dispatchState({ type: "SET_SIDEBAR_OPEN", payload: false })}
                    className="ml-auto"
              ...
              <div className="flex-1 overflow-hidden">
                <VideoNavigationSidebar
                  ...
                  onVideoSelect={(videoId) => {
                    ...
                    if (state.isSmallScreen) dispatchState({ type: "SET_SIDEBAR_OPEN", payload: false })
                  }}
