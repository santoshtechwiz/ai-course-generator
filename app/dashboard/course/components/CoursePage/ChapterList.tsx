...
import { Chapter, FullCourseType } from "@/app/types";
...
export default function ChapterList({
  ...
  const progress = Math.round(
    (sortedCompletedChapters.length / totalChapters) * 100
  );
    ...
    if (currentChapter) {
      ...
      if (unitWithCurrentChapter) {
        setExpandedUnits((prev) =>
          Array.from(new Set([...prev, unitWithCurrentChapter.id.toString()]))
        );
      }
    }
  ...
  return (
    <Card className="w-full mx-auto border-primary/20 shadow-md flex flex-col h-full">
        ...
        <div className="space-y-6 flex-grow flex flex-col overflow-hidden">
          <div className="flex-shrink-0">
            ...
            <div className="flex justify-between text-sm text-muted-foreground mt-2">
              <span>
                {sortedCompletedChapters.length} of {totalChapters} chapters
                completed
              </span>
            ...
            >
              {sortedUnits.map((unit, unitIndex) => (
                <AccordionItem
                  ...
                  className="border rounded-lg px-6 shadow-sm transition-all duration-200 hover:shadow-md"
                >
                  <AccordionTrigger className="hover:no-underline py-4 group">
                    <div className="flex flex-col items-start gap-1">
                      <Badge
                        variant="secondary"
                        className="font-normal bg-primary/10 text-primary"
                      >
                        Unit {unitIndex + 1}
                      </Badge>
                      ...
                      {unit.chapters
                        ...
                        .map((chapter, chapterIndex) => {
                          ...
                          const isCurrentlyPlaying =
                            chapter.videoId === currentVideoId;
                          ...
                          return (
                            <button
                              ...
                              onClick={() =>
                                chapter.videoId &&
                                handleVideoSelect(chapter.videoId, chapter.id)
                              }
                              className={cn(
                                ...
                                isCompleted && !isCurrentlyPlaying && !isSelected && "bg-green-100 text-green-900",
...
const ChapterIcon = React.memo<ChapterIconProps>(
  ({ isCompleted, isSelected, isCurrentlyPlaying }) => {
    if (isCurrentlyPlaying) {
      return <PlayCircle className="h-5 w-5 flex-shrink-0 text-yellow-500" />;
    }
    ...
    if (isSelected && !isCurrentlyPlaying) {
      return <BookOpen className="h-5 w-5 flex-shrink-0 text-black" />;
    }
