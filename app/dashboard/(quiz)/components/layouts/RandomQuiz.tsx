...
const QuizBackgroundPattern: React.FC<{ quizType: string }> = ({
  quizType,
}) => {
  const colorScheme =
    quizTypeColors[quizType as keyof typeof quizTypeColors] ||
    quizTypeColors.mcq;
  ...
  const patternElement =
    patterns[quizType as keyof typeof patterns] || defaultPattern;
...
const QuizCard: React.FC<{
  ...
  const Icon =
    quizTypeIcons[quiz.quizType as keyof typeof quizTypeIcons] || Brain;
  const colorScheme =
    quizTypeColors[quiz.quizType as keyof typeof quizTypeColors] ||
    quizTypeColors.mcq;
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              ...
              <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                ...
                >
                  <Icon
                    className={`h-5 w-5 sm:h-6 sm:w-6 ${colorScheme.icon}`}
                  />
                </motion.div>
  ...
  const prevCard = useCallback(() => {
    ...
    setTimeout(() => {
      setActiveCardIndex((prev) =>
        prev === 0 ? displayQuizzes.length - 1 : prev - 1
      );
      setIsTransitioning(false);
    }, 200);
  }, [displayQuizzes.length, isTransitioning]);
    ...
    <div className="h-full flex flex-col bg-gradient-to-br from-background via-background to-muted/20">
            ...
            {showFilters && (
              ...
              >
                ...
                >
                  ...
                  {quizTypes.map((type) => {
                    const colorScheme =
                      quizTypeColors[type.key as keyof typeof quizTypeColors] ||
                      quizTypeColors.mcq;
      ...
      <div className="flex-1 p-3 sm:p-4 lg:p-6">
        {isLoading ? (
          ...
          <div
            className={
              viewMode === "grid" ? "relative" : "space-y-3 sm:space-y-4"
            }
          >
            {viewMode === "grid" ? (
              <>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!isTransitioning) {
                            setIsTransitioning(true);
                            setActiveCardIndex(
                              (prev) => (prev + 1) % displayQuizzes.length
                            );
                          }
                        }}
                        disabled={isTransitioning}
