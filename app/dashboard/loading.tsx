...
export default function DashboardLoading() {
  return (
    <div className="flex flex-col gap-6 pt-24 items-center px-6 animate-in fade-in">
      <div className="h-8 w-48 rounded-none bg-muted animate-pulse" />
      <div className="h-4 w-80 max-w-full rounded-none bg-muted/70 animate-pulse" />
      <div className="grid gap-6 w-full max-w-6xl md:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="h-48 rounded-xl border bg-background/40 backdrop-blur-sm neo-shadow overflow-hidden flex flex-col"
          >
