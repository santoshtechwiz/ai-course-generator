...
import { FullCourseType, FullCourseUnit, FullChapter, CourseProgress, CourseQuestion } from "../types/types"
...
export async function getCourseData(slug: string): Promise<FullCourseType | null> {
  ...
  const fullCourse: FullCourseType = {
    ...
    courseUnits: course.courseUnits.map((unit): FullCourseUnit => ({
      id: unit.id,
      ...
      // Transform chapters
      chapters: unit.chapters.map((chapter): FullChapter => ({
        id: chapter.id,
    ...
    courseProgress: course.courseProgress.map((progress): CourseProgress => ({
      id: progress.id,
