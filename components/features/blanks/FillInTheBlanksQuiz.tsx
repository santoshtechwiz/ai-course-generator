...
export function FillInTheBlanksQuiz({ question, onAnswer, questionNumber, totalQuestions }: FillInTheBlanksQuizProps) {
  ...
  const progressiveHints = useMemo(() => {
    ...
    // First hint is always the length
    hints.push(`The answer has ${correctAnswer.length} characters.`)

    // Progressive character reveals
    for (let i = 1; i <= hintSteps; i++) {
      ...
      for (let j = 0; j < correctAnswer.length; j++) {
        if (j < revealedLength) {
          hintText += correctAnswer[j]
        } else {
          // Use underscore for spaces, otherwise use dot
          hintText += correctAnswer[j] === " " ? " " : "•"
        }
      }
    ...
    // Add the custom hints from the question
      ...
      <Card className="shadow-md border-t-4 border-t-primary">
        ...
        <CardContent className="pt-2 pb-6">
          <div className="space-y-6">
            {/* Fixed HTML structure to avoid <div> inside <p> */}
        ...
        <CardFooter className="flex justify-between items-center gap-4 border-t pt-4 pb-4 flex-col-reverse sm:flex-row">
          <Button
            variant="outline"
            size="sm"
